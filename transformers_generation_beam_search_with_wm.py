...
class BeamSearchScorer(BeamScorer):
    ...
    def process(
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                ...
                if (eos_token_id is not None) and (next_token.item() == eos_token_id):
                    ...
                    if token_type is not None:
                        if torch.sum(1 - token_type[batch_beam_idx]) > 0:
                            wm_filtered_input_ids = torch.masked_select(input_ids[batch_beam_idx], torch.gt(token_type[batch_beam_idx], 0))
                        else:
                            wm_filtered_input_ids = input_ids[batch_beam_idx]

                        beam_hyp.add(
                        wm_filtered_input_ids.clone(), # input_ids[batch_beam_idx].clone(),
                        next_score.item(),)
                    else:
                        beam_hyp.add(
            ...
            # TODO: fic cur_len to correspond to the seq len (w/o mem) with max score
