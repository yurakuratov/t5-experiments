...
class NNTrainer(FitTrainer):
    ...
    def train_on_batches(self, iterator: DataLearningIterator) -> None:
        ...
        while True:
            ...
            # get slice only for the first iteration over the dataset
            if (self.start_from_batch > 0) and (self.train_batches_seen == 0):
                # islice counts elements from 0-th
                batches_iterator = islice(iterator.gen_batches(self.batch_size, data_type='train'),
                          self.start_from_batch, None)

                log.info(f'Found: start_from_batch option, starting training from batch = {self.start_from_batch}')
                log.info(f'Found: self.train_batches_seen = {self.train_batches_seen}')
                self.train_batches_seen += self.start_from_batch
                log.info(f'Updated: self.train_batches_seen = {self.train_batches_seen}')
                log.info(f'Found: self.examples = {self.examples}')
                self.examples += self.batch_size * self.start_from_batch
                log.info(f'Updated: self.examples = {self.examples}')
                log.info(f'Found: self.epoch = {self.epoch}')
                self.epoch += self.train_batches_seen // int(iterator.samples_per_epoch / self.batch_size)
                log.info(f'Updated: self.epoch = {self.epoch}')
                
            else:
                batches_iterator = iterator.gen_batches(self.batch_size, data_type='train')
