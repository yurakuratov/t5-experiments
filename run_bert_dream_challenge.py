...
class DreamChallengeDataset(Dataset):
    def __init__(self, datafiles, tokenizer, x_field=0, label_field=1, max_seq_len=512, pad_to_max=True,
                 target_mean=11.147008459472662, target_std=2.3707070873733525):
        ...
        for f in datafiles:
            self.data = pd.concat([self.data, pd.read_csv(f, sep='\t', header=None)])
    ...
    def __getitem__(self, idx):
        ...
        features = DreamChallengeDataset.get_features(x, self.tokenizer, self.max_seq_len, self.pad_to_max)
        target = self.data[self.label_field][idx]
        target_std = (target - self.target_mean) / self.target_std
        targets = {'labels': np.array(target_std, dtype=np.float32)}
...
if __name__ == '__main__':
    ...
    train_dataset = DreamChallengeDataset(data_path, tokenizer, max_seq_len=args.input_seq_len, pad_to_max=True)
    ...
    if args.valid_data_path:
        ...
        valid_dataset = DreamChallengeDataset(valid_data_path, tokenizer, max_seq_len=args.input_seq_len,
                                              pad_to_max=True)
    ...
    if args.test_data_path:
        ...
        test_dataset = DreamChallengeDataset(test_data_path, tokenizer, max_seq_len=args.input_seq_len, pad_to_max=True)
    ...
    def keep_for_metrics_fn(batch, output):
        ...
        data['predictions'] = output['logits'].detach().squeeze()
        return data
    ...
    def metrics_fn(data):
        ...
        metrics['pearsonr2'] = stats.pearsonr(y, p)[0] ** 2
        metrics['spearman'] = stats.spearmanr(y, p)[0]
        return metrics
