...
def main():
    ...
    if args.valid_data_path:
        ...
        with accelerator.main_process_first():
            valid_data_index = get_indexed_dataset_(str(valid_data_path), args.data_impl,
                                                    skip_warmup=args.data_skip_warmup)
        valid_dataset = BertDataset(indexed_dataset=valid_data_index, masked_lm_prob=args.mlm_prob,
                                    ...
                                    max_seq_length=args.input_seq_len, mask_label_id=-100, seed=args.seed)
        valid_sampler = DistributedSampler(valid_dataset, rank=accelerator.process_index,
                                           num_replicas=accelerator.num_processes, shuffle=False)
    ...
    # accelerate (data_loaders are already distributed-aware)
